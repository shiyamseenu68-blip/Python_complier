/**
 * pyodideRunner2.ts — Browser-based Python execution using Pyodide with GitHub Pages CDN
 * 
 * Using GitHub Pages CDN which may be more reliable on Vercel than jsdelivr
 */
...
async function getPyodide() {
  ...
  const cdnSources = [
    ...
    'https://pyodide-cdn.githubusercontent.com/v0.23.4/full/',
    'https://unpkg.com/pyodide@0.23.4/full/'
  ];
...
sys.stderr = ErrorCapture()
      ...
      lastError = e;
      console.warn(`Failed to load Pyodide from ${indexURL}:`, e);
      pyodide = null;
      loadingPromise = null;
    }
  }
  
  throw lastError || new Error('Failed to load Pyodide from all CDN sources');
}
